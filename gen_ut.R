...
# eqSpVec              # (beg, end, N) { a vector from beg to end. N equally spaced steps (so N+1 components)
...
# cfDirContent         # cfDirContent(dirA, dirB, nameFrag, ext='', NdigOnly=0)
                       # outputs file lists list(inAnotB, inBoth, inBnotA)
                       # NdigOnly to compare the first few digits of file names only.

try(library(ppcor))    # Just because I use it all the time !!

#
tol1 <- 1e-10;  # additive tolerance so that 1 - (1-tol1) appox== tol 
tol2 <- 1e-12;  # R just about manages w this.
...
# Just to facilitate regression analyses - - - - - - - - - - - - - - - - - - - - - -
# 
lmpvals <- function(modelobject){ 
# Return vector with the p values for the whole regressor and each dv beta separately,
#        preg     pbeta1     pbeta2  ...
# from https://stackoverflow.com/questions/5587676/pull-out-p-values-and-r-squared-from-a-linear-regression
  ...
  pbeta = c();
  try({
    pbeta = anova(modelobject)$`Pr(>F)` ;
    l = length(pbeta); pbeta = pbeta[-l];  # remove this which is usually a padding NA
    hd = c('preg',paste('pbeta',1:(l-1),sep=''),'adjrsq');
    p = c(p,pbeta,NA);
    names(p) = hd;
  })
  # Also attempt to concatenate ajusted prop. of variance explained:
  try({p[length(p)] = summary(modelobject)$adj.r.squared })
  return(p)
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -
eqSpVec <- function(beg, end, N) {
  # create a vector from beg to end comprizing of N equally spaced steps
  # (so N+1 components)
  step <- (end-beg)/N
  V <- c(0,N+1)
  V[1] <- beg
  for (i in 1:N){ V[i+1] <- V[i]+step }
  return(V)   
}
# -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -   -
...
# - - - - - - - - - - - -  - - - - - - - - - - -
cfDirContent <- function (dirA, dirB, nameFrag, ext='', NdigOnly=0){
# outputs cfdir = list(inAnotB, inBoth, inBnotA)
# Find the files *'nameFrag'*'ext' that are in 
# dirA which aren't in dirB and vice versa; return two df's with
...
regExpFname <-  glob2rx(paste("*",nameFrag,"*",ext, sep=''))
    
lsA = dir(path=dirA,regExpFname)
lsB = dir(path=dirB,regExpFname)

if (NdigOnly){
  for (i in 1:length(lsA)){ lsA[[i]] =  substr(lsA[[i]],1,NdigOnly) }
  for (i in 1:length(lsB)){ lsB[[i]] =  substr(lsB[[i]],1,NdigOnly) }
}

inAnotB =  lsA[!vecTRUE(lsA %in% lsB)]
inBnotA =  lsB[!vecTRUE(lsB %in% lsA)]
inBoth  =  lsA[vecTRUE(lsA %in% lsB)]

cfdir = list(inAnotB=inAnotB, inBoth=inBoth, inBnotA=inBnotA)
return(cfdir)

} # end of whole function cfDirContent
